  ...
  def create_new_order_for_new_client(attrs,_wallet_from, wallet_to ) do
   attrs =  FunctionsCommons.convert_params(attrs)
    ...
    Repo.transaction(fn ->
      ...
      {:ok, _bucket_debt} <-  Buckets.create_new_debt_namaxa_for_deposit_client(attrs, order),
      {:ok, _bucket_debt1} <-  Buckets.create_new_debt_sellet_to_namaxa_for_deposit_client(attrs, order) do
        order
      else
        {:error, changeset} ->
          changeset
